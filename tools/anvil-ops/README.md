...
> Status: 1.0.5 on npm. Commands: `doctor`, `metrics`, `audit`, `insights`, `submit`, `sites` (multi-site registry), `mcp` (stdio MCP server). Works with or without wrangler.toml (falls back to `.env` SITE_URL / PUBLIC_CF_BEACON_TOKEN).
