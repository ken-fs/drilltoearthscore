...
description: "AnvilWiki 分三层:代码层(几乎别动)、配置层(每游戏改一次)、内容层(天天加)。用改动决策树 30 秒定位任何需求该落在哪,附数据流和 Astro 的六个坑(5→7 迁移全重验)。"
manual: dev
order: 1
icon: lucide:layers
...
## Astro 的六个坑(只在改代码层时才需要读)

这六条全是实测踩出来的,并在 Astro 5→6→7 迁移中逐条重验,改内容层/配置层用不到:
